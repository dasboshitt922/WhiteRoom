 ...
 v2f vert(appdata v) {
     ...
     float extra_range = max(_FogDistance.z / _ProjectionParams.z, 1.0);
     o.cameraToFarPlane = farPlaneWorld * extra_range - _WorldSpaceCameraPos + _FlickerFreeCamPos;
 ...
 void applyDiffusion(inout half4 sum) {
     ...
     sum.rgb += _SunColor.rgb * (pow(sunAmount, 8.0) * sum.a); // = lerp(sum.rgb, _SunColor, pow(sunAmount, 8.0) * sum.a );
 }
 #endif
...
//       dir.w = min(dir.w, 200);    // maximum iterations could be clamped to improve performance under some point of view, most of time got unnoticieable
     ...
     dir.z   /= _FogData.y;
     float4 ft4 = float4(fogCeilingCut, 0); 
     ft4.xy  *= _FogData.w;
...
#if FOG_USE_XY_PLANE
     #if FOG_AREA_SPHERE || FOG_AREA_BOX
         float2 areaCenter = _FogAreaPosition.xy;
...
#else
     #if FOG_AREA_SPHERE || FOG_AREA_BOX
         float2 areaCenter = _FogAreaPosition.xz;
         areaCenter  *= _FogData.w;
...
//               if (voidDistance>1.1) continue; // already cropped outside
         ...
         if (ng.a > 0) {
            fgCol =  _Color * half4((1.0-ng.a*_DeepObscurance).xxx, ng.a);
                 ...
                 float t = dir.w * _VolumetricFogSunShadowsData.w;
                     ...
                     half shadowAtten = GetLightAttenuation(shadowPos);
                 #else
                     // *** Custom Sun shadows ***
