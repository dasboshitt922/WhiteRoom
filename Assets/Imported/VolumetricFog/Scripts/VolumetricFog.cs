...
namespace VolumetricFogAndMist {
    ...
    public partial class VolumetricFog : MonoBehaviour {
        ...
        const string VFM_BUILD_HINT = "VFMBuildHint1042RC1";
        ...
        [SerializeField]
        FOG_VISIBILITY_SCOPE _visibilityScope = FOG_VISIBILITY_SCOPE.Global;
        ...
        [SerializeField]
        Bounds _visibilityVolume = new Bounds(Vector3.zero, new Vector3(1000, 1000, 1000));
        ...
        public List<VolumetricFog> renderingInstances { get { return fogRenderInstances;  } }

        /// <summary>
        /// Returns a list of registered fog instances
        /// </summary>
        /// <value>The instances.</value>
        public List<VolumetricFog> instances { get { return fogInstances; } }
                ...
                if (fogInstancesCount > 1) {
                    ...
                    if (needResort) {
                        ...
                        fogInstances.Sort((VolumetricFog x, VolumetricFog y) => {
                            ...
                            if (overlaps || x._fogAreaSortingMode == FOG_AREA_SORTING_MODE.Altitude || y._fogAreaSortingMode == FOG_AREA_SORTING_MODE.Altitude) {
        ...
        internal void DoOnRenderImage(RenderTexture source, RenderTexture destination) {
            ...
            for (int k = 0; k < registeredInstancesCount; k++) {
                ...
                if (fogInstance != null && fogInstance.isActiveAndEnabled && fogInstance.density > 0) {
                    if (fogInstance._visibilityScope == FOG_VISIBILITY_SCOPE.Global || fogInstance._visibilityVolume.Contains(camPos)) {
				...
				for (int k = 0; k < pointLightParams.Length; k++) {
					...
					if (pointLightComponent != null) {
                        ...
                        if (pointLightParams[k].lightParams == null) {
                            pointLightParams[k].lightParams = pointLightParams[k].light.GetComponent<VolumetricFogLightParams>();
                            if (pointLightParams[k].lightParams == null) {
                                pointLightParams[k].lightParams = pointLightParams[k].light.gameObject.AddComponent<VolumetricFogLightParams>();
                            }
                        }
...
#pragma warning restore 0429
        ...
        void UpdateMaterialHeights(Camera mainCamera) {
            ...
            fogMat.SetVector("_FogData", new Vector4(currentFogAltitude, _height, 1.0f / (_density * temporaryProperties.density), scale));
        ...
        void UpdateMaterialPropertiesNow(Camera mainCamera) { 
            ...
            Vector4 fogStepping = new Vector4(1.0f / (_stepping + 1.0f), 1 / (1 + _steppingNear), _edgeThreshold, _dithering ? _ditherStrength * 0.01f : 0f);
...
#pragma warning restore 0162
        ...
        void UpdateWindSpeedQuick() {
            ...
            fogMat.SetVector("_FogWindDir", new Vector3(windSpeedAcum.x % noiseTextureSize, 0, windSpeedAcum.z % noiseTextureSize));
            // sky speed
        ...
        void UpdateScatteringData(Camera mainCamera) {
            ...
            if (viewportPos.z < 0) {
                ...
                fogMat.SetVector("_FogScatteringData2", new Vector4(_lightScatteringIllumination, _lightScatteringDecay, _lightScatteringJittering, _lightScatteringEnabled && LIGHT_DIFFUSION_ENABLED ? 1.2f * _lightScatteringDiffusion * night * sunLightIntensity : 0));
        ...
        void UpdateTexture() {
            ...
            switch (_lightingModel) {
                ...
                case LIGHTING_MODEL.Natural:
                    lastRenderSettingsAmbientLight = RenderSettings.ambientLight;
                    lastRenderSettingsAmbientIntensity = RenderSettings.ambientIntensity;
                    updatingTextureLightColor = Color.Lerp(lastRenderSettingsAmbientLight, currentLightColor * fogIntensity + lastRenderSettingsAmbientLight, _lightIntensity);
                    skyHazeLightColor = Color.Lerp(lastRenderSettingsAmbientLight, _skyColor * fogIntensity + lastRenderSettingsAmbientLight, _lightIntensity);
                    break;
                case LIGHTING_MODEL.SingleLight:
                    lastRenderSettingsAmbientLight = Color.black; 
                    lastRenderSettingsAmbientIntensity = RenderSettings.ambientIntensity;
                    updatingTextureLightColor = Color.Lerp(lastRenderSettingsAmbientLight, currentLightColor * fogIntensity, _lightIntensity);
                    skyHazeLightColor = Color.Lerp(lastRenderSettingsAmbientLight, _skyColor * fogIntensity, _lightIntensity);
                    break;
            }
        ...
        void SetPointLightMaterialProperties(Camera mainCamera) {
            ...
            for (int k = 0; k < maxLights; k++) {
                ...
                if (!sunShadowsActive) { // when sun shadows are enabled, fogCeilingCut is not displaced in the shader
                    pos.y -= _baselineHeight;
                }
                float range = pointLightParams[k].range * pointLightParams[k].rangeMultiplier * _pointLightInscattering / 25f; // note: 25 comes from Unity point light attenuation equation
                float multiplier = pointLightParams[k].intensity * pointLightParams[k].intensityMultiplier * _pointLightIntensity;
